...
public class SQLQueries {
	...
	public static final String UPDATE_NEW_TOKEN_AGAINST_AUTHZ_CODE = "UPDATE IDN_OAUTH2_AUTHORIZATION_CODE SET "
			+ "TOKEN_ID=? WHERE AUTHORIZATION_CODE= (SELECT " + "AUTHORIZATION_CODE FROM "
			+ "IDN_OAUTH2_AUTHORIZATION_CODE WHERE TOKEN_ID=? )";
	...
	public static final String DEACTIVATE_AUTHZ_CODE_AND_INSERT_CURRENT_TOKEN = "UPDATE IDN_OAUTH2_AUTHORIZATION_CODE SET "
			+ "STATE='INACTIVE', TOKEN_ID=?" + " WHERE AUTHORIZATION_CODE= ?";
	...
	public static final String RETRIEVE_ACTIVE_ACCESS_TOKEN = "SELECT ca.CONSUMER_KEY, at.AUTHZ_USER, at.TENANT_ID, at.USER_DOMAIN, ats.TOKEN_SCOPE, at.TIME_CREATED, at.REFRESH_TOKEN_TIME_CREATED, "
			+ "at.VALIDITY_PERIOD, at.REFRESH_TOKEN_VALIDITY_PERIOD, at.USER_TYPE, at.REFRESH_TOKEN, at.TOKEN_ID, at.GRANT_TYPE, at.SUBJECT_IDENTIFIER FROM (SELECT CONSUMER_KEY_ID, TOKEN_ID, "
			+ "AUTHZ_USER, TENANT_ID, USER_DOMAIN, TIME_CREATED, REFRESH_TOKEN_TIME_CREATED, VALIDITY_PERIOD, REFRESH_TOKEN_VALIDITY_PERIOD, USER_TYPE, REFRESH_TOKEN, GRANT_TYPE, SUBJECT_IDENTIFIER "
			+ "FROM IDN_OAUTH2_ACCESS_TOKEN WITH (INDEX(IDN_OAUTH2_ACCESS_TOKEN_IDX2), NOLOCK) WHERE ACCESS_TOKEN = ? AND TOKEN_STATE = 'ACTIVE' ) at, IDN_OAUTH_CONSUMER_APPS ca, "
			+ "IDN_OAUTH2_ACCESS_TOKEN_SCOPE ats WHERE at.CONSUMER_KEY_ID = ca.ID AND at.TOKEN_ID = ats.TOKEN_ID";
	...
	public static final String RETRIEVE_ACCESS_TOKEN_VALIDATION_DATA_MSSQL = "SELECT ACCESS_TOKEN, AUTHZ_USER, IDN_OAUTH2_ACCESS_TOKEN_SELECTED.TENANT_ID, "
			+ "USER_DOMAIN, IDN_OAUTH2_ACCESS_TOKEN_SCOPE.TOKEN_SCOPE, TOKEN_STATE, REFRESH_TOKEN_TIME_CREATED, REFRESH_TOKEN_VALIDITY_PERIOD, "
			+ "IDN_OAUTH2_ACCESS_TOKEN_SELECTED.TOKEN_ID, GRANT_TYPE, SUBJECT_IDENTIFIER FROM (SELECT TOP 1 ACCESS_TOKEN, AUTHZ_USER, TENANT_ID, "
			+ "USER_DOMAIN, TOKEN_STATE, REFRESH_TOKEN_TIME_CREATED, TOKEN_ID REFRESH_TOKEN_VALIDITY_PERIOD, TOKEN_ID, GRANT_TYPE, SUBJECT_IDENTIFIER FROM "
			+ "$accessTokenStoreTable WITH(NOLOCK) WHERE CONSUMER_KEY_ID = (SELECT ID FROM IDN_OAUTH_CONSUMER_APPS WHERE CONSUMER_KEY = ?) AND REFRESH_TOKEN = ? "
			+ "ORDER BY TIME_CREATED DESC) IDN_OAUTH2_ACCESS_TOKEN_SELECTED LEFT JOIN IDN_OAUTH2_ACCESS_TOKEN_SCOPE ON IDN_OAUTH2_ACCESS_TOKEN_SELECTED.TOKEN_ID "
			+ "= IDN_OAUTH2_ACCESS_TOKEN_SCOPE.TOKEN_ID";
